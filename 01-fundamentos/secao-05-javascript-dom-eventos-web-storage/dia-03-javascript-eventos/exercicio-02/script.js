...
function createDays() {
  ...
  for (let i = 0; i < decemberDaysList.length; i += 1) {
    ...
    if (
      dayItem.innerText === '4' ||
      dayItem.innerText === '11' ||
      dayItem.innerText === '18' ||
      dayItem.innerText === '25'
    ) {
      dayItem.classList.add('friday');
    }

    if (
      dayItem.innerText === '24' ||
      dayItem.innerText === '25' ||
      dayItem.innerText === '31'
    ) {
...
function addTarefa(string, cor) {
  const taskColor = addCorTarefa(cor)
  const tarefa = document.createElement('span');
  tarefa.innerText = string;
  tarefa.appendChild(taskColor)
  divTarefas.appendChild(tarefa);
}

addTarefa('organizar anotações', 'red');
addTarefa('Refatorar códigos', 'green');
...
function mudaCorDia() {
  ...
  dias.forEach((dia) => {
    dia.addEventListener('click', () => {
      dia.style.color = cor.style.backgroundColor;
    });
  });
}
